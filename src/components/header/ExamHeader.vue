  <template>
    <div class="container" :style="{backgroundColor: headerColor}">
      <div class="logo-wrap" v-if="!showLeftMenu">
        <img src="/logos/logo-exam-exam.svg" alt="logo" class="logo-img" />
      </div>
  ...
  <script>
    ...
    export default {
      ...
      computed: {
        ...mapState("examUser", ["name", "avatar", "role", "token"]),
      ...
      methods: {
        ...mapActions("examUser", ["getInfo", "logout"]),
        logoutHandler() {
          this.logout()

          setTimeout(()=> {
            this.$router.push({ name: "exam-login" });
          }, 300)
        },
      },
      mounted() {
